...
export function useProgress() {
  ...
  const noteFor = useCallback((key: string): ProblemNote => notes[key] ?? { solution: "", stuck: "" }, [notes]);

  /** দুটো ঘরই ফাঁকা হলে key মুছে যায় — ফাঁকা নোট জমে না */
  const setNote = useCallback(
    (key: string, field: keyof ProblemNote, value: string) =>
      setNotes((prev) => {
        const current = prev[key] ?? { solution: "", stuck: "" };
        const updated = { ...current, [field]: value };
        const next = { ...prev };
        if (!updated.solution.trim() && !updated.stuck.trim()) delete next[key];
