...
import { findProblem, problemKeyFor, type ProblemRef } from "./patterns";
...
export type TaskProblem = ProblemRef & {
  /**
   * কাজের লেখায় প্যাটার্নের নম্বর লেখা আছে কি না। মিশ্র অনুশীলনে লেখা থাকে না —
   * তখন প্যাটার্নের লিংক টিক দেওয়ার পরে দেখায়, আগে দেখালে চেনার অনুশীলনটাই
   * নষ্ট। 🧠 Interleaving
   */
...
export type Task = {
  ...
  repeat: boolean;
  /** `(LC 15 · ১.১ …)` বা `(GfG · ১০.১)` থাকলে — কোন প্রবলেম, কোন প্যাটার্ন */
...
export type Block = {
  num: number;
  /** ফাইলনাম থেকে, নম্বর বাদে — `01-foundation.md` → "foundation" */
  slug: string;
  /** H1 হুবহু, e.g. "ব্লক ১ — ভিত্তি" */
  title: string;
  /** H1-এর "— "-এর পরের অংশ */
  name: string;
...
/** `(LC 15)`, `(LC 15 · ১.১ Two Pointers)`, `(GfG · ১০.১)` */
const PROBLEM_RE = /\((?:LC\s+([০-৯0-9]+)|GfG)(?:\s+·\s+([০-৯0-9]+\.[০-৯0-9]+)[^)]*)?\)/;
const BOLD_RE = /\*\*(.+?)\*\*/;

function parseProblem(dayCode: string, raw: string): TaskProblem | null {
  const match = PROBLEM_RE.exec(raw);
  if (!match) return null;
  const lc = match[1] ? Number(toAsciiDigits(match[1])) : null;
  const name = BOLD_RE.exec(raw)?.[1] ?? "";
  const ref = findProblem(problemKeyFor(lc, name));
  /* ভুল নম্বর বা নাম build-এই ধরা পড়ুক — সাইটে ভাঙা লিংক নয় */
  if (!ref) throw new Error(`docs: দিন ${dayCode}-এর প্রবলেম (${match[0]}) patterns/-এ নেই`);
  const patternId = match[2] ? toAsciiDigits(match[2]) : null;
  if (patternId && patternId !== ref.patternId) {
    throw new Error(`docs: দিন ${dayCode}-এ ${match[0]} — প্রবলেমটা patterns/-এ ${ref.patternId}-এ`);
  }
  return { ...ref, patternShown: patternId !== null };
}
...
function parseTask(dayCode: string, raw: string): Task {
  ...
  return {
    ...
    repeat: raw.includes("🔁"),
    problem: parseProblem(dayCode, raw),
  };
}
...
/** প্রবলেমের key → plan-এর যে দিনগুলোয় আছে, প্যাটার্নের পাতার জন্য */
export function getProblemDays(): Record<string, { code: string; label: string }[]> {
  const map: Record<string, { code: string; label: string }[]> = {};
  for (const day of getDays()) {
    for (const task of day.tasks) {
      if (!task.problem) continue;
      const list = (map[task.problem.key] ??= []);
      if (!list.some((item) => item.code === day.code)) list.push({ code: day.code, label: day.label });
    }
  }
  return map;
}
